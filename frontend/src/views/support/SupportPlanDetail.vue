...
// 조회상태: readonly → 수정이력, 결과조회, 수정 (+ plan_result e0_00이면 승인,보완,반려)
  ...
  <div class="support-plan-detail card shadow-sm border-radius-lg mb-4">
    <div class="card-body">
      ...
      <div class="detail-fields mb-4">
        ...
        <div v-if="plan_result === 'e0_99' || plan_result === 'e0_80'" class="mb-3">
          <label class="form-label text-sm text-body mb-1">{{ plan_result === 'e0_99' ? '반려' : '보완' }}</label>
          <textarea
            class="form-control form-control-sm support-plan-textarea"
            rows="2"
      ...
      <div class="actions d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
        ...
        <!-- 조회상태: 반려(e0_99)=수정이력만, 승인(e0_10)=수정이력+결과조회, 보완(e0_80)=수정이력+수정+승인+보완+반려, 검토대기(e0_00)=전부 -->
        <template v-if="isViewMode()">
          <button
            v-if="plan_result === 'e0_10' || plan_result === 'e0_00'"
            type="button"
          ...
          <button
            v-if="plan_result === 'e0_80' || plan_result === 'e0_00'"
            type="button"
            class="btn btn-sm btn-primary"
            @click="startEdit"
          >
        ...
        <template v-else-if="isInputMode()">
          <button
            ...
            @click="emit('approval-request', { planCode: plan_code, title: titleLocal, content: contentLocal })"
          >
            승인요청
          </button>
          <button type="button" class="btn btn-sm btn-outline-secondary" @click="emit('request-cancel', plan_code)">
            취소
          </button>
        </template>
        <!-- 수정상태: 편집 중 + 내용 있음 → 수정완료, 취소 -->
        <template v-else-if="isEditMode()">
          <button type="button" class="btn btn-sm btn-success" @click="onEditComplete">
            수정완료
          </button>
          <button type="button" class="btn btn-sm btn-outline-secondary" @click="emit('request-cancel', plan_code)">
            취소
